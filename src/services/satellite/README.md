...
An AI-powered satellite image analysis tool that detects and quantifies environmental changes using NASA satellite imagery and Claude Vision API.

## Features

- **Satellite Image Fetching**: Download before/after images from NASA Earth Imagery API
...
├── satellite_fetcher.py           # NASA API integration
...
├── satellite_config.py            # Configuration and region presets
...
2. **NASA API Key** (Optional but recommended)
   - Get free key at: https://api.nasa.gov/ (instant approval)
   - Add to `.env` file
   - Without it, you'll use DEMO_KEY (limited to 30 requests/hour)
...
- NASA API might be down (check https://api.nasa.gov/)
- DEMO_KEY is rate-limited (30 requests/hour) - get free API key
- Some regions may not have recent imagery

### "Image not found at date"
- NASA may not have imagery for exact date requested
- Try nearby dates or check available dates first
- Some locations have limited satellite coverage

### Import Errors
- Install required packages: `pip install anthropic python-dotenv requests`
- Ensure Python 3.8+ is installed
- Run from project root directory

### High Cloud Cover Warning
- Images with >50% cloud cover will show warning
- Consider trying different dates
- Check `cloud_score` in results (0-1, lower is better)
...
Before analyzing, check which dates have good imagery:
...
# Shows dates sorted by cloud cover (best first)
for date in dates[:5]:
    print(f"{date['date']} - Cloud cover: {date['cloud_score']*100:.1f}%")
```
...
- API responses are cached to save costs on repeated analyses
- Results are saved with timestamps to prevent overwriting
- Images are organized by location name
- Cloud cover is automatically checked and reported
- High cloud cover (>50%) will trigger warnings
...
- **Satellite Imagery**: NASA Earth Imagery API (Landsat 8)
